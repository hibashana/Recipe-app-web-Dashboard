  ...
  const addRecipeToCategory = async (rcpid,name) => {
    try {
      ...
      if (response.status === 200) {
        ...
        toast.error(`Failed to add recipe ${name} to the category.`);
      }
    } catch (error) {
      toast.error(`An error occurred: ${error.message}`);
      console.error(`An error occurred: ${error.message}`);
  ...
  const deleteRecipeFromCategory = async (id, name) => {
    ...
    if (confirmDelete) {
      try {
        ...
        if (response.status === 200) {
          toast.success(`Recipe ${id} has been removed from the category.`);
          console.log(`Recipe ${id} has been removed from the category.`);
          // setTimeout(() => {
          //   window.location.reload();
          // }, 3000); // Reload the page after 3 seconds
        } else {
          toast.error(`Failed to remove recipe ${id} from the category.`);
          console.error(`Failed to remove recipe ${id} from the category.`);
        }
      } catch (error) {
        toast.error(`An error occurred: ${error.message}`);
        console.error(`An error occurred: ${error.message}`);
